...
export default function AttendancesPage() {
  ...
  const safeStopAndClear = useCallback(async (clearInstance: boolean = true) => {
    ...
    try {
      try { await scannerRef.current.stop(); } catch (err) { console.log("Scanner stop warning:", err); }
      if (clearInstance) {
        try { await scannerRef.current.clear(); } catch (err) { console.log("Scanner clear warning:", err); }
      }
    } finally {
      setScanning(false);
      setCameraActive(false);
      if (clearInstance) scannerRef.current = null;
  ...
  const handleScan = useCallback(
    async (decodedText: string) => {
      ...
      if (scannerRef.current && cameraActive) {
        try { await scannerRef.current.pause(true); } catch (err) { console.log("Could not pause scanner:", err); }
      }
      ...
      try {
        ...
        try {
          ...
          setError("QR inválido. El usuario debe generar un nuevo código desde su perfil.");
          setScanning(false);
          return;
        }

        if (!qrData.userId || !qrData.timestamp || !qrData.token || !qrData.expiresAt) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setTimeout(() => {
              if (scannerRef.current && cameraActive) {
                try { scannerRef.current.resume(); } catch (err) { console.log("Could not resume scanner:", err); }
              }
            }, 2000);
          } else {
            if (res.status === 409) {
              setDuplicateInfo({ fecha: data.fecha || new Date().toLocaleString("es-CO"), usuario: data.usuario || "Usuario" });
  ...
  const stopScanner = useCallback(() => { void safeStopAndClear(); }, [safeStopAndClear]);
  ...
  const getAvailableCameras = useCallback(async () => {
    try {
      ...
      if (devices && devices.length > 0) {
        ...
        if (!selectedCameraId && cameraIds.length > 0) setSelectedCameraId(cameraIds[0]);
        return cameraIds;
      }
      return [];
    } catch (error) {
  ...
  const startScanner = useCallback(async () => {
    ...
    setCameraError(""); setScanning(false); setError(""); setSuccess("");
    setDuplicateInfo(null); setLastScanned(null);

    const cameras = await getAvailableCameras();
    if (!selectedCameraId || !cameras.includes(selectedCameraId)) setSelectedCameraId(cameras[0]);
    ...
    try {
      ...
      const getQrBoxSize = () => {
        ...
        return { width: Math.max(200, Math.floor(w * 0.7)), height: Math.max(200, Math.floor(w * 0.7)) };
      };
      ...
      await scanner.start(
        ...
        (errorMessage) => { console.log("Scanner error (normal):", errorMessage); },
      );
      ...
      setTimeout(() => {
        ...
        } catch (e) { console.warn("Could not set video attributes:", e); }
      }, 300);
        ...
        try {
          ...
          if (videoElement) {
            if (!videoElement.videoWidth && !videoElement.videoHeight) {
              setTimeout(() => {
                if (!videoElement.videoWidth && !videoElement.videoHeight) handleRetry();
              }, 2000);
            }
          } else {
            setTimeout(() => {
              if (!document.querySelector("#reader video")) handleRetry();
            }, 1500);
          }
      ...
      if (err instanceof Error) {
        if (err.name === "NotAllowedError") msg += "Permiso denegado. Recarga la página y permite el acceso.";
        else if (err.name === "NotFoundError") msg += "No se encontró ninguna cámara disponible.";
        else if (err.name === "NotSupportedError") msg += "Tu navegador no soporta el acceso a la cámara.";
        else msg += "Error: " + err.message;
      } else {
        msg += "Error desconocido: " + err;
      }
      setCameraError(msg);
      setCameraActive(false);
      setWaitingForPermission(false);
  ...
  const handleRetry = useCallback(async () => {
    setError(""); setSuccess(""); setDuplicateInfo(null); setLastScanned(null);
    setCameraError(""); setWaitingForPermission(false);
    await safeStopAndClear();
    const readerElement = document.getElementById("reader");
    if (readerElement) readerElement.replaceChildren();
  ...
  const handleCameraChange = useCallback(
    async (cameraId: string) => {
      ...
      if (!cameraActive) { setSelectedCameraId(cameraId); return; }
      await safeStopAndClear();
      setCameraActive(false);
  ...
  useEffect(() => {
    const fetchCsrf = async () => {
      try {
        const response = await fetch("/api/auth/csrf-token", { method: "GET", credentials: "include" });
        if (response.ok) {
  ...
  useEffect(() => {
    const handleVisibility = () => { if (document.hidden) stopScanner(); };
    ...
    <div className="min-h-screen bg-variable-collection-fondo p-2 sm:p-4 lg:p-8 font-sans">
      <style jsx global>{`
        ...
        @media (max-width: 640px) {
          #reader { min-height: 250px !important; max-height: 60vh !important; }
      ...
      <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6">
        ...
        {/* ── Page Header ── */}
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 pb-2 border-b border-[#2E2E2E]">
          <div>
